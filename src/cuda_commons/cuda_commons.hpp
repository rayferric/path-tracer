...
inline void check_cuda_error(cudaError_t result, const char *stmt, const char *file, int line) {
	if (result != cudaSuccess) {
		std::cerr << "CUDA Error: " << stmt << " - " << cudaGetErrorString(result) << " at " << file << ":" << line << std::endl;
...
enum class cuda_memcpy_kind { host_to_device, device_to_host, device_to_device, host_to_host };
