...
void scene::load_hdri(const std::filesystem::path &file) {
	...
	textures[num_textures] = {.data_offset = new_offset, .width = static_cast<uint32_t>(width), .height = static_cast<uint32_t>(height), .channels = static_cast<uint8_t>(channels), .format = texture_format::hdr};
